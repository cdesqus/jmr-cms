...
function requireAdminSecret(ctx) {
    const secret = process.env.JAMORA_ADMIN_API_SECRET;
    if (!secret)
        return true;
    if (ctx.get("x-jamora-admin-secret") === secret)
        return true;
    ctx.status = 401;
...
function contentData(body) {
    ...
    if ("certifications" in body)
        data.certifications = parseListText(body.certifications);
    return data;
}
function productData(body, mode) {
    const data = {};
    ...
    if (typeof body.sku === "string")
        data.sku = body.sku.trim().toUpperCase();
    if (typeof body.botanical === "string")
        data.botanical = body.botanical.trim();
    if (["energy", "digestion", "balance"].includes(asString(body.category)))
        data.category = body.category;
    if (typeof body.priceCents === "number")
        data.priceCents = Math.max(0, Math.round(body.priceCents));
    if (typeof body.stock === "number")
        data.stock = Math.max(0, Math.round(body.stock));
    if (typeof body.minStock === "number")
        data.minStock = Math.max(0, Math.round(body.minStock));
    if (typeof body.maxStock === "number")
        data.maxStock = Math.max(0, Math.round(body.maxStock));
    if (typeof body.featured === "boolean")
        data.featured = asBoolean(body.featured);
    if (typeof body.tagline === "string")
        data.tagline = body.tagline.trim();
    if (typeof body.description === "string")
        data.description = body.description.trim();
    if (typeof body.howToUse === "string")
        data.howToUse = body.howToUse.trim();
    if (typeof body.netWeight === "string")
        data.netWeight = body.netWeight.trim();
    if ("ingredients" in body)
        data.ingredients = parseListText(body.ingredients);
    if ("allergens" in body)
        data.allergens = parseListText(body.allergens);
    if ("benefits" in body)
        data.benefits = parseListText(body.benefits);
    if ("certifications" in body)
        data.certifications = parseListText(body.certifications);
    if ("gradient" in body)
        data.gradient = parseListText(body.gradient);
    ...
    if (mode === "create") {
        data.name || (data.name = "New Jamora Product");
        data.slug || (data.slug = asString(data.name).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""));
        data.sku || (data.sku = `JM-${asString(data.slug).replace(/-/g, "").slice(0, 12).toUpperCase()}`);
        data.category || (data.category = "energy");
        data.priceCents || (data.priceCents = 0);
        data.netWeight || (data.netWeight = "30 sachets · 90 g");
        data.stock || (data.stock = 0);
        data.minStock || (data.minStock = 10);
        data.maxStock || (data.maxStock = 100);
        data.gradient || (data.gradient = ["#e2913f", "#c25a2b"]);
    }
    return data;
}
...
function normalizeOrderItems(items) {
    return items.map((item) => {
        const qty = Math.max(0, Math.round(asNumber(item === null || item === void 0 ? void 0 : item.qty)));
        const unitPriceCents = Math.max(0, Math.round(asNumber(item === null || item === void 0 ? void 0 : item.unitPriceCents)));
        return {
            productId: asString(item === null || item === void 0 ? void 0 : item.productId),
            sku: asString(item === null || item === void 0 ? void 0 : item.sku),
            slug: asString(item === null || item === void 0 ? void 0 : item.slug),
            name: asString(item === null || item === void 0 ? void 0 : item.name, "Product"),
            qty,
            unitPriceCents,
            lineTotalCents: Math.max(0, Math.round(asNumber(item === null || item === void 0 ? void 0 : item.lineTotalCents, qty * unitPriceCents))),
        };
...
function promotionData(body, mode) {
    const data = {};
    if (typeof body.name === "string")
        data.name = body.name.trim();
    if (typeof body.code === "string")
        data.code = normalizePromotionCode(body.code);
    ...
    if (mode === "create") {
        data.name || (data.name = asString(data.code, "New promotion"));
        data.discountType || (data.discountType = "percentage");
        data.discountValue || (data.discountValue = 10);
        data.minimumSpendCents || (data.minimumSpendCents = 0);
        data.usageLimit || (data.usageLimit = 0);
        data.perCustomerLimit || (data.perCustomerLimit = 0);
        data.productSlugs || (data.productSlugs = []);
        if (!("active" in data))
            data.active = true;
    }
    return data;
}
function promotionWindowError(data) {
    ...
    if (startsAt !== null && !Number.isFinite(startsAt))
        return "Promotion start date is invalid.";
...
async function evaluatePromotion(input) {
    ...
    const promotions = await strapi.documents("api::promotion.promotion").findMany({
        fields: [
            "documentId",
            "name",
            "code",
    ...
    if (!promotion || !promotion.active)
        return { error: "Coupon is invalid or inactive." };
    const now = Date.now();
    ...
    const productSlugs = Array.isArray(promotion.productSlugs)
        ? promotion.productSlugs.map((slug) => asString(slug).toLowerCase())
        : [];
    const eligibleSubtotalCents = productSlugs.length === 0
        ? subtotalCents
        : input.items.reduce((sum, item) => {
            return productSlugs.includes(asString(item === null || item === void 0 ? void 0 : item.slug).toLowerCase())
                ? sum + Math.max(0, asNumber(item === null || item === void 0 ? void 0 : item.lineTotalCents))
                : sum;
        }, 0);
...
async function recordInventoryMovement(input) {
    var _a, _b;
    if (!((_a = input.product) === null || _a === void 0 ? void 0 : _a.documentId) || input.delta === 0)
        return;
...
const batchDocuments = () => strapi.documents("api::inventory-batch.inventory-batch");
const reservationDocuments = () => strapi.documents("api::stock-reservation.stock-reservation");
const returnDocuments = () => strapi.documents("api::return-request.return-request");
const notificationDocuments = () => strapi.documents("api::notification-log.notification-log");
...
async function recordAudit(ctx, input) {
    ...
    await strapi.documents("api::audit-log.audit-log").create({
        data: { ...input, ...adminIdentity(ctx), details: (_a = input.details) !== null && _a !== void 0 ? _a : {} },
    });
}
...
async function releaseReservationAllocations(reservation) {
    const allocations = Array.isArray(reservation === null || reservation === void 0 ? void 0 : reservation.allocations) ? reservation.allocations : [];
    for (const allocation of allocations) {
        ...
        await batchDocuments().update({
            ...
            data: {
                reservedQuantity: Math.max(0, asNumber(batch.reservedQuantity) - asNumber(allocation.qty)),
            },
        });
    }
}
...
async function createStockReservation(items, email = "") {
    var _a, _b, _c, _d, _e;
    ...
    const virtuallyReserved = new Map();
    for (const reservation of activeReservations) {
        for (const allocation of Array.isArray(reservation.allocations) ? reservation.allocations : []) {
            if (!allocation.batchDocumentId) {
                ...
                virtuallyReserved.set(slug, ((_a = virtuallyReserved.get(slug)) !== null && _a !== void 0 ? _a : 0) + asNumber(allocation.qty));
            }
        }
    }
    const allocations = [];
    ...
    for (const item of items) {
        ...
        const usable = batches.filter((batch) => {
            return batch.status === "active" && (!batch.expiryDate || new Date(batch.expiryDate).getTime() >= Date.now());
        });
        let remaining = qty;
        if (usable.length > 0) {
            for (const batch of usable) {
                const planned = (_c = (_b = batchUpdates.get(batch.documentId)) === null || _b === void 0 ? void 0 : _b.reservedQuantity) !== null && _c !== void 0 ? _c : asNumber(batch.reservedQuantity);
        ...
        const trackedBatchQuantity = batches.reduce((sum, batch) => sum + asNumber(batch.quantity), 0);
        const available = Math.max(0, asNumber(product === null || product === void 0 ? void 0 : product.stock) - trackedBatchQuantity - ((_d = virtuallyReserved.get(slug)) !== null && _d !== void 0 ? _d : 0));
        if (!product || available < remaining) {
            return { error: `Only ${qty - remaining + available} unit(s) of ${item.name} are available.` };
        }
        virtuallyReserved.set(slug, ((_e = virtuallyReserved.get(slug)) !== null && _e !== void 0 ? _e : 0) + remaining);
        allocations.push({
...
async function consumeReservation(token, orderNumber) {
    if (!token)
        return { allocations: [] };
    await expireReservations();
    ...
    if (!reservation)
        return { error: "Stock reservation expired or was already used." };
    ...
    for (const allocation of allocations) {
        ...
        const batch = await batchDocuments().findOne({ documentId: allocation.batchDocumentId });
        if (!batch)
            return { error: `Batch ${allocation.batchNumber} is unavailable.` };
...
async function enrichBatchAllocations(value) {
    const allocations = Array.isArray(value) ? value : [];
    return Promise.all(allocations.map(async (allocation) => {
        if ((allocation === null || allocation === void 0 ? void 0 : allocation.productionDate) || !(allocation === null || allocation === void 0 ? void 0 : allocation.batchDocumentId)) {
            return allocation;
        }
...
async function decrementProductStock(items, reference) {
    for (const item of items) {
        const slug = asString(item.slug);
        const qty = Math.max(0, Math.round(asNumber(item.qty)));
        if (!slug || qty <= 0)
        ...
        const product = products[0];
        if (!(product === null || product === void 0 ? void 0 : product.documentId) || typeof product.stock !== "number")
            continue;
...
exports.default = {
    ...
    async mockPaidOrder(ctx) {
        ...
        const promotionResult = body.promotionCode
            ? await evaluatePromotion({
                ...
                items,
                subtotalCents,
                email: (_b = body.customer) === null || _b === void 0 ? void 0 : _b.email,
            })
            : null;
        ...
        const discountCents = (_c = promotionResult === null || promotionResult === void 0 ? void 0 : promotionResult.discountCents) !== null && _c !== void 0 ? _c : 0;
        const totalCents = Math.max(0, subtotalCents - discountCents + shippingCents);
        const reservationToken = asString(body.reservationToken);
        let reservedAllocations = [];
        if (reservationToken) {
            await expireReservations();
        ...
        const storefrontUrl = ((_e = (_d = process.env.JAMORA_STOREFRONT_URL) !== null && _d !== void 0 ? _d : process.env.PUBLIC_URL) !== null && _e !== void 0 ? _e : "http://localhost:3095").replace(/\/$/, "");
        const costRatio = Number((_f = process.env.JAMORA_COST_RATIO) !== null && _f !== void 0 ? _f : DEFAULT_COST_RATIO);
        const estimatedProfitCents = Math.max(0, Math.round(totalCents * (1 - (Number.isFinite(costRatio) ? costRatio : DEFAULT_COST_RATIO))));
        const order = await strapi.documents("api::order.order").create({
            data: {
                orderNumber,
                trackingNumber: asString(body.trackingNumber),
                carrier: asString(body.carrier, "Jamora EU Fulfilment"),
                customerName: asString((_g = body.customer) === null || _g === void 0 ? void 0 : _g.name),
                email: asString((_h = body.customer) === null || _h === void 0 ? void 0 : _h.email, "customer@example.com"),
                status: "paid",
                currency: "eur",
                subtotalCents,
                shippingCents,
                discountCents,
                promotionCode: (_j = promotionResult === null || promotionResult === void 0 ? void 0 : promotionResult.promotion) === null || _j === void 0 ? void 0 : _j.code,
                totalCents,
                ...
                shippingAddressText: asString((_k = body.customer) === null || _k === void 0 ? void 0 : _k.address, asString((_l = body.shippingAddress) === null || _l === void 0 ? void 0 : _l.address)),
        ...
        if (reservationToken) {
            ...
            if ("error" in consumed)
                strapi.log.warn(consumed.error);
        }
        if ((_m = promotionResult === null || promotionResult === void 0 ? void 0 : promotionResult.promotion) === null || _m === void 0 ? void 0 : _m.documentId) {
        ...
        await logNotification({
            recipient: asString((_o = body.customer) === null || _o === void 0 ? void 0 : _o.email, "customer@example.com"),
    ...
    async trackOrder(ctx) {
        ...
        const batchAllocations = await enrichBatchAllocations(order.batchAllocations);
        ctx.body = {
            order: {
                ...order,
                batchAllocations,
                statusHistory: parseHistory(order.statusHistory).filter((event) => (event === null || event === void 0 ? void 0 : event.type) === "status"),
            },
        };
    },
    async summary(ctx) {
        ...
        const costRatio = Number((_a = process.env.JAMORA_COST_RATIO) !== null && _a !== void 0 ? _a : DEFAULT_COST_RATIO);
        ...
        const revenueCents = paidOrders.reduce((sum, order) => sum + asNumber(order.totalCents), 0);
        const profitCents = paidOrders.reduce((sum, order) => {
        ...
        const todayRevenueCents = paidOrders
            .filter((order) => new Date(order.createdAt).toISOString() >= today)
            .reduce((sum, order) => sum + asNumber(order.totalCents), 0);
        ctx.body = {
    ...
    async adminCustomers(ctx) {
        ...
        for (const order of orders) {
            ...
            if (!key)
                continue;
            const existing = (_a = customers.get(key)) !== null && _a !== void 0 ? _a : {
                email,
    ...
    async adminUpdateOrder(ctx) {
        ...
        const existingHistory = parseHistory(existing.statusHistory);
        ...
        if (note && (!status || status === existing.status)) {
        ...
        if (typeof body.carrier === "string")
            data.carrier = body.carrier.trim();
        if (typeof body.estimatedDelivery === "string")
            data.estimatedDelivery = body.estimatedDelivery;
        if (Array.isArray(body.fulfilmentChecklist)) {
            data.fulfilmentChecklist = body.fulfilmentChecklist.map((item) => ({
            ...
            const allChecked = data.fulfilmentChecklist.length > 0 && data.fulfilmentChecklist.every((item) => item.checked);
            data.packedAt = allChecked && asString(body.packedBy, asString(existing.packedBy)).trim()
                ? new Date().toISOString()
                : null;
        }
        if (typeof body.packedBy === "string")
            data.packedBy = body.packedBy.trim();
        const finalChecklist = Array.isArray(data.fulfilmentChecklist)
            ? data.fulfilmentChecklist
            : Array.isArray(existing.fulfilmentChecklist)
                ? existing.fulfilmentChecklist
                : [];
        if (status === "shipped" && (finalChecklist.length === 0 || !finalChecklist.every((item) => item.checked))) {
            ctx.status = 400;
        ...
        await recordAudit(ctx, { action: "order.updated", entityType: "order", entityId: order.documentId, entityLabel: existing.orderNumber, details: { status: data.status, trackingNumber: data.trackingNumber, packedBy: data.packedBy } });
        if (status && status !== existing.status) {
            const templates = {
                processing: { template: "order_processing", subject: `Jamora order ${existing.orderNumber} is being prepared` },
                shipped: { template: "order_shipped", subject: `Jamora order ${existing.orderNumber} has shipped` },
                fulfilled: { template: "order_delivered", subject: `Jamora order ${existing.orderNumber} was delivered` },
                refunded: { template: "order_refunded", subject: `Jamora order ${existing.orderNumber} was refunded` },
            };
            ...
            if (notification) {
                await logNotification({
                    ...
                    ...notification,
                    payload: { status, trackingNumber: asString(body.trackingNumber, asString(existing.trackingNumber)) },
                });
            }
        }
        ctx.body = { ok: true, order };
    ...
    async adminUpdateProduct(ctx) {
        ...
        if (typeof data.stock === "number" && data.stock !== existing.stock) {
            await recordInventoryMovement({
                ...
                delta: data.stock - asNumber(existing.stock),
    ...
    async adminCreateProduct(ctx) {
        ...
        if (asNumber(product.stock) > 0) {
            await recordInventoryMovement({
                product,
                delta: asNumber(product.stock),
                balanceAfter: asNumber(product.stock),
    ...
    async adminCreateInventoryBatch(ctx) {
        ...
        if (body.adjustProductStock !== false && quantity > 0) {
            const balanceAfter = asNumber(product.stock) + quantity;
            await strapi.documents("api::product.product").update({
                documentId: product.documentId,
    ...
    async adminUpdateInventoryBatch(ctx) {
        ...
        const body = (_a = ctx.request.body) !== null && _a !== void 0 ? _a : {};
        const data = {};
        if (["active", "quarantined", "depleted"].includes(asString(body.status)))
            data.status = body.status;
        if (typeof body.certificateUrl === "string")
            data.certificateUrl = body.certificateUrl.trim();
    ...
    async adminCreateReturn(ctx) {
        ...
        const returnRequest = await returnDocuments().create({
            data: {
                ...
                email: order.email,
                items: Array.isArray(body.items) && body.items.length > 0 ? body.items : (_b = order.items) !== null && _b !== void 0 ? _b : [],
    ...
    async adminCreatePromotion(ctx) {
        ...
        if (!requireAdminSecret(ctx))
            return;
        const data = promotionData((_a = ctx.request.body) !== null && _a !== void 0 ? _a : {}, "create");
        if (!data.code) {
            ctx.status = 400;
        ...
        const duplicates = await strapi.documents("api::promotion.promotion").findMany({
            fields: ["documentId"],
        ...
        const promotion = await strapi.documents("api::promotion.promotion").create({
    ...
    async adminUpdatePromotion(ctx) {
        ...
        if (!requireAdminSecret(ctx))
            return;
        const data = promotionData((_a = ctx.request.body) !== null && _a !== void 0 ? _a : {}, "update");
        const existing = await strapi.documents("api::promotion.promotion").findOne({
            documentId: ctx.params.documentId,
        ...
        if (data.code && data.code !== existing.code) {
            ...
            if (duplicates.some((promotion) => promotion.documentId !== existing.documentId)) {
                ctx.status = 409;
                ctx.body = { error: "Promotion code already exists." };
                return;
            }
        }
        const promotion = await strapi.documents("api::promotion.promotion").update({
            documentId: ctx.params.documentId,
            data,
        });
    ...
    async adminUpload(ctx) {
        ...
        if (!requireAdminSecret(ctx))
            return;
        const files = (_a = ctx.request.files) === null || _a === void 0 ? void 0 : _a.files;
        if (!files) {
            ctx.status = 400;
    ...
    async adminUpdateContent(ctx) {
        ...
        const content = (existing === null || existing === void 0 ? void 0 : existing.documentId)
