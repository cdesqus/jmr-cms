...
The storefront reads products, prices, and inventory from the Medusa **Store API**. The Admin dashboard (your CMS) is served by the Medusa backend.
...
Jamora products, and creates the admin user. Then wire the storefront to the
backend with the generated publishable key:

```bash
docker compose logs medusa | grep PUBLISHABLE_KEY     # copy the pk_... value
#  → paste it into .env as MEDUSA_PUBLISHABLE_KEY
docker compose up -d storefront                       # restart storefront with the key
```
...
- The storefront reads the Medusa key **server-side at runtime** (not baked into the
  build), so you don't rebuild the image to set it — just restart with the env var.
- Until `MEDUSA_PUBLISHABLE_KEY` is set, the storefront serves its built-in mock
  catalogue, so the site is never down.
...
#   Log in with the admin user created during setup (see below).
...
- The storefront's checkout button is currently a stub — wire it to Medusa's cart →
  payment-session → Stripe flow once the module is configured.
...
- Env: `NEXT_PUBLIC_MEDUSA_BACKEND_URL` (the deployed backend URL) and
