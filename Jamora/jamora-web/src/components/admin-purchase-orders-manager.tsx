...
const NEXT: Partial<Record<PurchaseOrder["status"], PurchaseOrder["status"]>> = { draft: "ordered", ordered: "in_transit" };
...
function PurchaseOrderTable({ orders, advance }: { orders: PurchaseOrder[]; advance: (order: PurchaseOrder) => void }) {
  return <section className="overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="hidden grid-cols-[1fr_1fr_0.8fr_0.8fr_1.6fr] gap-4 border-b border-slate-200 bg-slate-50 px-5 py-3 text-xs font-bold uppercase tracking-[0.1em] text-slate-500 lg:grid"><span>PO</span><span>Supplier</span><span>Status</span><span>Volume</span><span className="text-right">Documents & action</span></div><div className="divide-y divide-slate-100">{orders.map((order) => { const totalUnits = order.items.reduce((sum, item) => sum + item.quantity, 0); const receivedUnits = order.items.reduce((sum, item) => sum + (item.receivedQuantity ?? 0), 0); return <article key={order.documentId} className="grid gap-3 px-5 py-4 lg:grid-cols-[1fr_1fr_0.8fr_0.8fr_1.6fr] lg:items-center"><div><p className="font-bold">{order.poNumber}</p><p className="text-xs text-slate-500">{order.expectedDate ? `Expected ${order.expectedDate}` : "No expected date"}</p></div><div><p className="text-sm font-semibold">{order.supplierName}</p><p className="text-xs text-slate-500">{order.items.length} product line{order.items.length === 1 ? "" : "s"}</p></div><div><span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold capitalize ${order.status === "partially_received" ? "bg-amber-50 text-amber-700" : order.status === "received" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-700"}`}>{order.status.replaceAll("_", " ")}</span><p className="mt-1 text-xs text-slate-500">{order.receipts?.length ?? 0} receipt{order.receipts?.length === 1 ? "" : "s"}</p></div><div><p className="text-sm font-bold">{receivedUnits} / {totalUnits} received</p><p className="truncate text-xs text-slate-500">{order.items.map((item) => item.productName).join(", ")}</p></div><div className="flex flex-wrap justify-start gap-2 lg:justify-end"><DocumentLink href={`/admin/purchase-orders/${order.documentId}/print`} label="PO PDF" /><DocumentLink href={`/admin/purchase-orders/${order.documentId}/labels?format=unit`} label="Unit labels" /><DocumentLink href={`/admin/purchase-orders/${order.documentId}/labels?format=carton`} label="Carton labels" />{NEXT[order.status] ? <button type="button" onClick={() => advance(order)} className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white">Mark {NEXT[order.status]?.replaceAll("_", " ")}</button> : null}</div></article>; })}{orders.length === 0 ? <div className="p-10 text-center"><p className="font-semibold text-slate-700">No purchase orders yet</p><p className="mt-1 text-sm text-slate-500">Create the first production order for an approved supplier.</p></div> : null}</div></section>;
}
