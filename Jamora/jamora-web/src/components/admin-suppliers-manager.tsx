...
import type { AdminProduct, Supplier } from "@/lib/admin-api";

const EMPTY_FORM = { name: "", code: "", contactName: "", email: "", phone: "", country: "Indonesia", notes: "", active: true };

export function AdminSuppliersManager({ suppliers, products }: { suppliers: Supplier[]; products: AdminProduct[] }) {
  ...
  return <div className="space-y-4">
    ...
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
      <div className="hidden grid-cols-[1fr_1.15fr_1.2fr_0.6fr_auto] gap-3 border-b border-slate-200 bg-slate-50 px-5 py-3 text-xs font-bold uppercase tracking-[0.12em] text-slate-500 md:grid">
        <span>Supplier</span><span>Contact</span><span>Products</span><span>Country</span><span>Status</span>
      </div>
      <div className="divide-y divide-slate-100">
        {suppliers.map((supplier) => { const suppliedProducts = products.filter((product) => product.supplierDocumentIds?.includes(supplier.documentId)); return <article key={supplier.documentId} className="grid gap-3 px-5 py-4 md:grid-cols-[1fr_1.15fr_1.2fr_0.6fr_auto] md:items-center">
          ...
          <div>{suppliedProducts.length > 0 ? <><p className="text-sm font-semibold text-slate-700">{suppliedProducts.length} product{suppliedProducts.length === 1 ? "" : "s"}</p><p className="truncate text-xs text-slate-500" title={suppliedProducts.map((product) => product.name).join(", ")}>{suppliedProducts.map((product) => product.name).join(", ")}</p></> : <p className="text-xs font-semibold text-amber-700">No products assigned</p>}</div>
    ...
    {open ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby="supplier-dialog-title" className="max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto rounded-xl bg-white shadow-2xl">
        <header className="flex items-start justify-between border-b border-slate-200 px-6 py-5">
          <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-blue-600">Master data</p><h2 id="supplier-dialog-title" className="mt-1 text-2xl font-bold">Add supplier</h2><p className="mt-1 text-sm text-slate-500">Register the partner here, then assign its products from Product master data.</p></div>
